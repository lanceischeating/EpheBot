...
from main import REQUIRED_ROLE, GUILD_ID
...
class SlashModeration(commands.Cog):
    ...
    def is_authorized(self, interaction: discord.Interaction) -> bool:
        ...
        has_required_role = REQUIRED_ROLE in user_roles or interaction.user.top_role > REQUIRED_ROLE
        has_admin_roles = any(role.permissions.administrator for role in user_roles)

        return is_owner or has_admin_roles or has_required_role
...
#timeout
    ...
    async def timeout(self, interaction: discord.Interaction, target: discord.Member, duration: int, duration_unit: str,
        ...
        if not SlashModeration.is_authorized(interaction):
            await interaction.response.send_message(
            f"You do not have the required role, {REQUIRED_ROLE} to use this command", ephemeral=ephemeral)
            return
...
#Set Moderation Role
    ...
    @app_commands.command(
        name="setmodrole",
        description="Sets the moderation role for the server",)
    @app_commands.describe(
        role="The role to set as the moderation role",
        ephemeral="Set whether the bot response is visible to other users TRUE or FALSE")
    async def set_mod_role(self, interaction: discord.Interaction, role: discord.Role, ephemeral: bool = False) -> None:
        if not SlashModeration.admin_authorized(interaction):
            await interaction.response.send_message(
            f"Only an administrator or server owner is authorized to use this command", ephemeral=ephemeral)
            return
        await interaction.response.send_message(f"Set {role.mention} as the moderation role", ephemeral=ephemeral)

    @app_commands.guilds(discord.Object(id=GUILD_ID))
    @app_commands.command(
        name="getmodrole",
        description="Gets the moderation role for the server",)
    async def get_mod_role(self, interaction: discord.Interaction) -> None:
        if not SlashModeration.is_authorized(interaction):
            await interaction.response.send_message(
            f"You do not have the required role, {REQUIRED_ROLE} to use this command", ephemeral=True)
            return
        role = REQUIRED_ROLE
        await interaction.response.send_message(f"{role.mention} is the moderation role for this server.")